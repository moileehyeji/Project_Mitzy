...
modelpath = f'./mitzy/data/modelcheckpoint/23_deltime_data_ResNet.hdf5'
er,mo,lr = callbacks(modelpath)

model.fit(x_train, y_train, epochs=10, validation_data=(x_val,y_val), callbacks=[er,mo,lr], batch_size = 200)#, callbacks = [es, re, mo]) # validation_splitr 기본 0.2
...
df['test'] = y_pred
df.to_csv('./mitzy/data/csv/23_deltime_data_ResNet.csv',index=False)

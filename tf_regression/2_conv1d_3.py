...
def build_model(params, n1, n2):    
    ...
    #3. 훈련
    modelpath =f"./data/hdf5/2_conv1d_3_{n1}_{n2}.hdf5"
...
n1 = 0
n2 = 0
optimizers = [RMSprop, Nadam, Adadelta, Adamax, Adagrad, SGD, Adam]#, Nadam, Adadelta, Adamax, Adagrad, SGD, Adam]
activations = ['tanh','swish','elu', 'relu', 'selu']#,'swish','elu','LeakyReLU','tanh', 'relu', 'selu']
...
for opti in optimizers:
    params['optimizer'] = opti
    n1 +=1
    for acti in activations:    
        params['activation'] = acti 
        n2 +=1
        
        params, model = build_model(params, n1, n2)
        total_param.append(params)    
#==========================================================================
...
# 모델로드
model = load_model(f"./data/hdf5/2_conv1d_3_{n1}_{n2}.hdf5")

# 평가
evaluate = evaluate_list(model)
total_eval.append(evaluate)

print('total_param : ', total_param)
print('total_score  : ', total_eval)

...
for train_index, test_index in kfold.split(x_train): 
   ...
   # 2. 모델구성
    ...
    # 3. 컴파일 훈련
    modelpath = './data/hdf5/6_compare_encoding'+str(num)+'.hdf5'
    es= EarlyStopping(monitor='val_loss', patience=10)
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=5, factor=0.5, verbose=1)
    cp =ModelCheckpoint(filepath='modelpath', monitor='val_loss', save_best_only=True, mode='auto')

    model.compile(loss='mse', optimizer='RMSprop', metrics='mae')
    model.fit(x_train1, y_train1, epochs=1000, batch_size=94, validation_data=(x_val,y_val), callbacks=[es,reduce_lr,cp] )

    # 4. 평가, 예측
    loss, mae = model.evaluate(x_test1, y_test1, batch_size=94)
...
# LSTM 윈도우 없음
# r2 :  [0.6340823255023798, 0.64445572148192, 0.6453822166851155]
# RMSE :  [2.347922484839391, 2.314402599289244, 2.31138513876559]
# loss :  [4.4946513175964355, 4.456268787384033, 4.4922895431518555]
# 32982.913674초 걸렸습니다.
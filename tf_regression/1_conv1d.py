...
def build_model(drop=0.5, optimizer=RMSprop, kernel_size=2, learning_rate=0.1, activation = PReLU):
    inputs = Input(shape = (6,1),name = 'input')
    x = Conv1D(filters=256,kernel_size=kernel_size,padding='same',activation=activation(),name='hidden1')(inputs)
    x = Dropout(drop)(x)
    x = Conv1D(filters=64,kernel_size=kernel_size,padding='same',activation=activation(),name='hidden3')(x)
    x = Dropout(drop)(x)
...
def create_hyperparameter() : 
    batchs = [60]#50,70, 80]
    optimizers = [RMSprop, Nadam]#, Adadelta, Adamax, Adagrad, SGD, Adam]
    dropout = [0.3]#, 0.2, 0.1]
    # filters = [100]#10,50,100,200,300]
    kernel_size = [2]#, 3]
    activations = [swish,PReLU,elu]#,LeakyReLU,tanh, relu, elu, selu, ]
    learning_rate = [0.1]#, 0.005, 0.001]
    return {'batch_size' : batchs, 'optimizer' : optimizers, 'drop': dropout, 'kernel_size' : kernel_size, 'learning_rate' : learning_rate, 'activation':activations}  

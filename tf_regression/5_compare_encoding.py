...
print(df[['category', 'dong']].head())
#===========
df['category_n_rows'] = df['category'].map(df.groupby('category').size())
df['dong_n_rows'] = df['dong'].map(df.groupby('dong').size())
...
df['category'] = df.apply(lambda x:smoothing(x['category_n_rows'], x['category']), axis=1)
df['dong'] = df.apply(lambda x:smoothing(x['dong_n_rows'], x['dong']), axis=1)
...
for train_index, test_index in kfold.split(x_train): 
   ...
   # 2. 모델구성
    ...
    # 3. 컴파일 훈련
    modelpath = './data/hdf5/5_compare_encoding'+str(num)+'.hdf5'
...
# LSTM 윈도우 없음
# r2 :  [0.0, 0.0, 0.0]
# RMSE :  [43058.45345778573, 43240.46972300928, 41135.6923743828]
# loss :  [1853992832.0, 1869976320.0, 1692601088.0]
# 9897.655736초 걸렸습니다.
...
for train_index, valid_index in kfold.split(x_train1):
    ...
    #2. 모델구성
    model = GridSearchCV(LGBMRegressor(), parameters, cv=kfold)
    ...
    # 모델저장
    joblib.dump(model.best_estimator_, f'./data/h5/LGBM_kfold_{num}.pkl')

    # 모델로드
    model = joblib.load(f'./data/h5/LGBM_kfold_{num}.pkl')
